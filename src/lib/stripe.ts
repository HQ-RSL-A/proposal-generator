...
import { STRIPE_MAX_PRODUCT_NAME_CHARS, STRIPE_MIN_CHARGE_CENTS } from "@/lib/constants";
...
export function buildLineItems(
  ...
  return checkout.lineItems.map((li) => {
    ...
    if (li.label.length > STRIPE_MAX_PRODUCT_NAME_CHARS) {
      throw new Error(
        `Refusing to build a Stripe line whose label exceeds ${STRIPE_MAX_PRODUCT_NAME_CHARS} chars: "${li.label.slice(0, 40)}..." (RSL-36)`
      );
    }
    return {
      quantity: 1,
